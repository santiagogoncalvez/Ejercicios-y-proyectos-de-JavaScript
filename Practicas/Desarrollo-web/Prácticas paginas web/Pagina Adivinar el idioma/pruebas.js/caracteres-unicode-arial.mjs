const codesUnicode = `0	Basic Latin	0000-007F	
...
function simboloAleatorioLeng(probabilidades) {
    ...
    if (String.fromCodePoint(numeroDentroDelRango).length > 1) {
        numeroDentroDelRango = Math.floor(
            Math.random() * (rangoSeleccionado[1] - rangoSeleccionado[0]) +
            rangoSeleccionado[0]
        );
...
const regExp = /\d{1,3}\s+[\w\sáéíóúüÁÉÍÓÚÜ`\-\(\)]+?\s[0-9A-F]{4,5}-[0-9A-F]{4,5}/g;
const regExpHexadecimal = /[0-9A-F]{4,5}-[0-9A-F]{4,5}/g;

const arrCodes = codesUnicode.match(regExp);

const rangesArialStrings = arrCodes.map((element) => {
    return element.match(regExpHexadecimal)[0]
});

export const rangesArial = convertirArrayHexadecimal(rangesArialStrings).filter(range => {
    return range[0] < 65535 && range[1] < 65535;
});

// console.log(rangesArial.some(element => {
//     return element[0] >= 65535 || element[1] >= 65535;
// }))




// Función que devuelve un simbolo directamente dentro del rango de simbolos que acepta la fuente "Arial"

export const unicodeAleatorio = function () {
    let code = simboloAleatorioLeng(rangesArial);
